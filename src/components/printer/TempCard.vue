...
if (props.source === 'extruder'){
    sourceDefinition.value = {
        temperature: printer.extruder.temperature,
        label: 'Nozzle',
        icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="#ffffff"><path fill="#ffffff" d="M7 2h10v6h2v5h-2.5L13 17h-2l-3.5-4H5V8h2V2m3 20H2v-2h8a1 1 0 0 0 1-1v-1h2v1a3 3 0 0 1-3 3Z"/></svg>',
        maxTemp: printer.configfile.extruder.max_temp,
    }
} else if (props.source === 'bed'){
    sourceDefinition.value = {
        temperature: printer.bed.temperature,
        label: 'Bed',
        ...
        maxTemp: printer.configfile.heater_bed.max_temp,
    }
}
...
<template>
   <div class="flex items-center justify-between px-4 h-12 bg-base-300 rounded-xl overflow-hidden relative">
      <div class="label z-10">
        <div v-html="sourceDefinition.icon"></div>
        <h4>{{ sourceDefinition.label }}</h4>
      </div>
      <p class="z-10"><span class="font-bold">{{ sourceDefinition.temperature }}</span> °C</p>
      ...
      type="range" min="0" 
      :max="sourceDefinition.maxTemp" 
      :value="sourceDefinition.temperature">
  </div>
