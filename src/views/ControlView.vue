<script setup lang="ts">
import { ref } from 'vue'
import { usePrinterControlStore } from '../stores/printerControl.ts';
import CameraStream from '../components/CameraStream.vue';
import { usePrinterStore } from '../stores/printer.ts';

const control = usePrinterControlStore();
const printer = usePrinterStore();
</script>

<template>
    <h1>Printer Controls</h1>
    <CameraStream/>

    <p>Stepper X Enabled: {{ printer.stepperEnable.stepper_x }}</p>
    <p>All steppers Disabled: {{ printer.allSteppersDisabled }}</p>

    <div class="flex flex-col items-center gap-4">
        <div class="grid grid-cols-3 grid-rows-2 w-fit">
            <button class="btn col-start-2 row-start-1 aspect-square"><i class="ri-arrow-up-line"></i></button>
            <button class="btn col-start-1 row-start-2 aspect-square"><i class="ri-arrow-left-line"></i></button>
            <button class="btn col-start-2 row-start-2 aspect-square"><i class="ri-arrow-down-line"></i></button>
            <button class="btn col-start-3 aspect-square row-start-2"><i class="ri-arrow-right-line"></i></button>
        </div>

        <div class="flex gap-4">
            <button @click="control.homeAxis" class="btn btn-primary">Home Axis <i class="ri-home-2-fill"></i></button>
            <button @click="control.disableSteppers" class="btn btn-warning">Motors Off <i class="ri-hand"></i></button>
        </div>

        <div class="flex gap-4">
            <button class="btn btn-primary">Retract <i class="ri-arrow-down-line"></i></button>
            <button class="btn btn-primary">Extrude <i class="ri-arrow-up-line"></i></button>
        </div>
        <div class="flex flex-col gap-2">
            <div class="flex items-center gap-4">
                <p>Current: {{ printer.extruder.temperature.toFixed(2)  }} °C</p>
                <label class="input w-32">
                    <svg class="opacity-50 w-full" xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="#ffffff"><path fill="#ffffff" d="M7 2h10v6h2v5h-2.5L13 17h-2l-3.5-4H5V8h2V2m3 20H2v-2h8a1 1 0 0 0 1-1v-1h2v1a3 3 0 0 1-3 3Z"/></svg>
                    <input :value="printer.extruder.target" type="text" class="grow" />
                    <div class="opacity-50">
                        °C
                    </div>
                </label>
            </div>
            <div class="flex items-center gap-4">
                <p>Current: {{ printer.bed.temperature.toFixed(2) }} °C</p>
                <label class="input w-32">
                    <svg class="opacity-50 w-full" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="M532-131q-6 5-12.5 8t-14.5 3q-8 0-16-3.5t-14-9.5q-41-44-60.5-90T395-320q0-37 11-78t38-106q23-57 32-87.5t9-56.5q0-34-15-63.5T423-771q-6-6-9.5-14t-3.5-16q0-8 3-14.5t8-12.5q6-6 13.5-9t15.5-3q8 0 15 3t13 8q44 41 65.5 86t21.5 95q0 35-10.5 73.5T518-474q-25 60-34 92t-9 61q0 35 14.5 67.5T534-188q5 6 8 13t3 15q0 8-3 15.5T532-131Zm195 0q-6 5-12.5 8t-14.5 3q-8 0-16-3.5t-14-9.5q-41-44-60.5-89.5T590-319q0-37 11-79t38-106q23-57 32-87t9-56q0-34-15-64.5T618-771q-6-6-9-13.5t-3-15.5q0-8 2.5-14.5T616-827q6-6 14-9.5t16-3.5q8 0 14.5 3t12.5 8q44 41 65.5 86t21.5 95q0 35-10.5 73.5T713-473q-25 60-34 92t-9 60q0 35 15 68.5t45 65.5q5 6 7.5 13t2.5 14q0 8-3 16t-10 13Zm-390 0q-6 5-12.5 8t-14.5 3q-8 0-16-3.5t-14-9.5q-41-44-60.5-89.5T200-319q0-37 11-79t38-106q23-57 32-87t9-56q0-34-15-64.5T228-771q-7-6-10-13.5t-3-15.5q0-8 3-15t8-13q6-6 13.5-9t15.5-3q8 0 15 3t13 8q44 41 65.5 85.5T370-648q0 35-10 73.5T324-474q-25 60-34 92t-9 61q0 35 14.5 68.5T340-187q5 6 7.5 13t2.5 14q0 8-3 16t-10 13Z"/></svg>
                    <input :value="printer.bed.target" type="text" class="grow" />
                    <div class="opacity-50">
                        °C
                    </div>
                </label>
            </div>
        </div>
    </div>

</template>
